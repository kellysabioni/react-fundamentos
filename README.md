...
- Adicionar/Remevor manipuladores de evento
- Iniciar/controlar temporizadores 
- Sincronizar com mudanças externas

O useEffect é executado após a renderização e pode ser configurado para rodar tudo de uma vez, sempre que uma variável mudar ou em toda renderização, dependendo das dependências fornecidas.
...
- Link: componente especial para navegação entre as páginas através das rotas. Deve ser usado lugar da tag <a> (somente para links relativos/internos)
...
Estas props aceitam uma callback na sintaxe de arrow function ou então uma chamada (apenas o nome, sem parenteses) de uma função externa.
...
Props (abrevisação de proprieties/propriedades) são parâmetros que um componente pode receber.
...
- Criação de módulos para cada componente (exceto Footer)
...
- CSS inline criando um objeto do tipo CSSProperties e passando-o para o style do compnente
